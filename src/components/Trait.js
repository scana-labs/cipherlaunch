...
const Trait = ({ trait, categories, moveTrait, currentCategory }) => {
	...
	const handleChange = (newSelected) => {
		console.log('Handle change', newSelected)
		try {
			setSelected(newSelected)
			moveTrait(trait.id, newSelected.id)
		}
		catch (e) {
	...
	return (
		<div key={trait.id} className="group h-60">
			<div className={`w-full h-40 bg-${colors[trait.id % (colors.length - 1)]}-400 rounded-lg overflow-hidden`} />
					...
					<div className="w-40">
						...
						<div className="mt-1 relative">
							<Listbox.Button className="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
								<span className="block truncate">{selected?.name || 'No category!'}</span>
