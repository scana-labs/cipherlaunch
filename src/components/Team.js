...
const Team = () => (
	<div className="mx-auto py-12 px-4 max-w-7xl sm:px-6 lg:px-8 lg:py-24">
		<div className="flex flex-col justify-center items-center w-fit mb-24">
		...
		<div className="flex mt-10 lg:col-span-2">
			{people.map((person, idx) => (
				<div
					key={`people-${idx}`}
					className="team-card bg-white group relative border border-gray-200 rounded-2xl flex flex-col overflow-hidden mx-5 shadow-xl"
				>
					<div className="flex flex-col items-center space-x-4 lg:space-x-6 p-10">
						<img className="w-48 h-48 rounded-md" src={person.imageUrl} alt="" />
						<div className="flex flex-col items-center font-medium text-lg">
							<div className="flex flex-col items-center h-24 mt-6">
								<h3 className="text-2xl font-bold">{person.name}</h3>
								<p className="text-align-center text-sm">{person.background}</p>
							</div>
							<p>{person.role}</p>
							<a className="twitter-btn text-sm" href={person.twitter} target="_blank" rel="noreferrer">
